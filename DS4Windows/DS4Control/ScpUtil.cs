...
namespace DS4Windows
{
    ...
    public class Global
    {
        ...
        public const int CONFIG_VERSION = 6;
    ...
    public class BackingStore
    {
        ...
        public bool SaveProfileOld(int device, string proName)
        {
            ...
            try
            {
                ...
                XmlNode xmlLSDeadZoneTypeRadial = m_Xdoc.CreateNode(XmlNodeType.Element, "LSDeadZoneTypeRadial", null); xmlLSDeadZoneTypeRadial.InnerText = lsModInfo[device].deadZoneTypeRadial.ToString(); rootElement.AppendChild(xmlLSDeadZoneTypeRadial);
                XmlNode xmlLSDeadZoneTypeAxial = m_Xdoc.CreateNode(XmlNodeType.Element, "LSDeadZoneTypeAxial", null); xmlLSDeadZoneTypeAxial.InnerText = lsModInfo[device].deadZoneTypeAxial.ToString(); rootElement.AppendChild(xmlLSDeadZoneTypeAxial);
                XmlNode xmlRSDeadZoneTypeRadial = m_Xdoc.CreateNode(XmlNodeType.Element, "RSDeadZoneTypeRadial", null); xmlRSDeadZoneTypeRadial.InnerText = rsModInfo[device].deadZoneTypeRadial.ToString(); rootElement.AppendChild(xmlRSDeadZoneTypeRadial);
                XmlNode xmlRSDeadZoneTypeAxial = m_Xdoc.CreateNode(XmlNodeType.Element, "RSDeadZoneTypeAxial", null); xmlRSDeadZoneTypeAxial.InnerText = rsModInfo[device].deadZoneTypeAxial.ToString(); rootElement.AppendChild(xmlRSDeadZoneTypeAxial);
            ...
            string propath = "", bool xinputChange = true, bool postLoad = true)
        {
            ...
            if (File.Exists(profilepath))
            {
                ...
                try
                {
                    Item = m_Xdoc.SelectSingleNode("/" + rootname + "/LSDeadZoneTypeRadial");
                    if (Enum.TryParse(Item?.InnerText ?? "", out bool temp))
                    {
                        lsModInfo[device].deadZoneTypeRadial = temp;
                    }
                }
                catch { }
                ...
                try
                {
                    Item = m_Xdoc.SelectSingleNode("/" + rootname + "/LSDeadZoneTypeAxial");
                    if (Enum.TryParse(Item?.InnerText ?? "", out bool temp))
                    {
                        lsModInfo[device].deadZoneTypeAxial = temp;
                    }
                }
                catch { }
                ...
                try
                {
                    Item = m_Xdoc.SelectSingleNode("/" + rootname + "/RSDeadZoneTypeRadial");
                    if (Enum.TryParse(Item?.InnerText ?? "", out bool temp))
                    {
                        rsModInfo[device].deadZoneTypeRadial = temp;
                    }
                }
                catch { }
                ...
                try
                {
                    Item = m_Xdoc.SelectSingleNode("/" + rootname + "/RSDeadZoneTypeAxial");
                    if (Enum.TryParse(Item?.InnerText ?? "", out bool temp))
                    {
                        rsModInfo[device].deadZoneTypeAxial = temp;
                    }
                }
                catch { }
